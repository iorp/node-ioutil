...
/*
add type regex on assignations
test anonymoes classs with enters
class extends
inclass captures 
function and class attributes, (static, async ... )
docblock extract
docblock create 
filter by level or so on 
*/
        ...
        vlcDefinition:(lines,line,attributes,options,level,parent)=>{
          ...
          var collected = null; 
            ...
            collected={
              ...
              'level':level, 
              'type':'vlc-definition',  
              'definer':line.text.match(/^\s*([^\s]+)/)?.[1],// first word
              'datatype':undefined,
        ...
        prototypeDefinition:(lines,line,attributes,options,level,parent)=>{ 
          ...
          if(closure){
            ...
            if(closure.matchInner.trim().length>0) 
             Object.assign(collected,this.extract(lines.text,options,{offset:closure.start+1,limit:closure.end-1},level+1,collected[options.nodeDataKeyName]))
