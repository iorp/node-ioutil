...
*/
   ...
   const TurboDocJs= class{ 
    ...
    static collect ={
      vlcDefinitionAssigned:(lines,line,attributes,options)=>{
        ...
        if(assignee.startsWith('"')||assignee.startsWith("'")){ 
          collected = TurboDocJs.collect.assigneeQuotedString(assignee,lines,line,attributes,options);  
        ...
        if(assignee.startsWith('`')){
          collected = TurboDocJs.collect.asigneeEnclosure('`','`',[],lines,line,attributes,options);  
          attributes.offset = collected[options.nodeDataKeyName]['end'] +2; //2 = 1 the new line, 1 for the next char 
        }
        else
        //## object assignee 
        ...
        if(TurboDocJs.REGEX.NUMBER.test(assignee)){
          collected = TurboDocJs.collect.assigneeNumber(assignee,line,attributes,options); 
          attributes.offset = collected[options.nodeDataKeyName]['end'] +2; //2 = 1 the new line, 1 for the next char 
        }
        else 
        //## anonymous function assignee       
          if(TurboDocJs.REGEX.ANONYMOUS_FUNCTION.test(assignee)){      
          collected = TurboDocJs.collect.assigneeAnonymousFunction(lines,line,attributes,options); 
          attributes.offset =  collected[options.nodeDataKeyName]['end'] +2; //2 = 1 the new line, 1 for the next char  
        }
        else
        //## anonymous class assignee       
          if(TurboDocJs.REGEX.ANONYMOUS_CLASS.test(assignee)){      
          collected = TurboDocJs.collect.assigneeAnonymousClass(lines,line,attributes,options); 
          attributes.offset =  collected[options.nodeDataKeyName]['end'] +2; //2 = 1 the new line, 1 for the next char  
        }
      ...
      prototypeDefinition:(lines,line,attributes,options)=>{ 
        ...
        if(!argsClosure){  console.error('collect.prototypeDefinition: Malformed function arguments at line ',line.index);   } 
      ...
      functionDefinition:(lines,line,attributes,options)=>{
        ...
        if(!argsClosure){
            console.error('collect.functionDefinition:Malformed function arguments at line ',line.index);   
            return;  
          } 
        var argsList = parseList(argsClosure.matchInner);
        ...
        if(closure){
          collected={ 
            ...
            ...this.recurse(lines.text,options,{offset:closure.start+1,limit:closure.end-1,level:level+1}) 
          }
         }else{
      ...
      assigneeAnonymousFunction:(lines,line,attributes,options)=>{
        ...
        var collected = null;
        var argsClosure = extractClosure(lines.text,{opener:'(',closer:')',offset: offset});
        console.log(888,lines.text.substr(offset-4,15))
        if(!argsClosure){  console.error('collect.assigneeAnonymousFunction: Malformed function arguments at line ',line.index); return;  } 
        var argsList = parseList(argsClosure.matchInner);
      ...
      tagLine:(lines,line,attributes,options,output)=>{

        // todo see this ...
        if(line.text.includes('todo')) {  
          if(!output.todo) output.todo=[];
          output.todo.push(line.text) ;
    ...
    static recurse(input,options={},attributes={},output={}){
     ...
     var i =attributes.offset;
      while(i<input.length){
        ...
        console.log(attributes.offset,i,line)
        // skip line comments, find custom taglines
        if(line.text.startsWith('//')){
          this.collect.tagLine(lines,line,attributes,options,output) 
             continue;
        }
    ...
    static extractFromString(input,options={}){
      var output = {}
      var recursion =this.recurse(input,options,null,output);
...
var r = TurboDocJs.extractFromString((test.str||""))
